...
class Event(models.Model):
    ...
    # An event time will also have a mode, can either be a whole day (WDE), morning/afternoon/evening event(MAEE) or a precise time event(PTE)
    WHOLE_DAY_EVENT = 'WDE'
    MORNING_AFTERNOON_EVENING_EVENT = 'MAEE'
    PRECISE_TIME_EVENT = 'PTE'
    EVENT_TYPE_CHOICES = (
        (WHOLE_DAY_EVENT, 'Whole Day Event'),
        (MORNING_AFTERNOON_EVENING, 'Morning Afternoon Evening Event'),
    ...
    timeslots = models.ManyToManyField(Timeslot, blank=True,null=True)
...
class Timeslot(models.Model):
    ...
    TIME_TYPE_CHOICES = (
        ...
        (MORNING_AFTERNOON_EVENING, 'Morning Afternoon Evening Time'),
        (PRECISE_TIME_TIME, 'Precise Time Time')
    )
    time_type=models.IntegerField(max_length=10, choices=TIME_TYPE_CHOICES, default=WHOLE_DAY_TIME)
